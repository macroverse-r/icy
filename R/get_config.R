...
#'
#' @export
get_config <- function(package = NULL,
                       user = "default",
                       origin = "local",
                       yaml_file = NULL,
    ...
    if (is.null(yaml_file)) {
        ...
        if (!file.exists(yaml_file)) {
            if (!grepl("[/\\\\]", yaml_file)) {
                ...
                yaml_file <- file.path(get_package_dir(package = package), yaml_file)
            }
            if (!file.exists(yaml_file)) {
...
.get_config_template <- function(package = NULL,
    ...
    if (is.null(yaml_file)) {
        ...
        if (!file.exists(yaml_file)) {
            if (!grepl("[/\\\\]", yaml_file)) {
                # Just a filename, try to find it in package
                yaml_file <- file.path(get_package_dir(package = package, user_dir = FALSE), yaml_file)
            }
            if (!file.exists(yaml_file)) {
...
.get_config_renviron <- function(package = NULL,
    ...
    if (!is.null(package)) {
        ...
        template_vars <- tryCatch({
            names(.get_config_template(package = package, user = user))
