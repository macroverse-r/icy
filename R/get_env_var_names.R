...
#' `get_config_path` function with the specified case format. The YAML file should 
...
#'   the function will search for a YAML file ending with `env_vars.yml` using `get_config_path`.
...
get_env_var_names <- function(package = NULL, yaml_file = NULL, case_format = "snake_case") {
  ...
  if (is.null(yaml_file)) {
    # Try to find the YAML file using get_config_path
    # Try primary method first (get_config_path)
    yaml_file_found <- FALSE
    
    # Try using get_config_path
    tryCatch({
      yaml_file <- get_config_path(package = package,
