...
.check_file_pairing <- function(fn_tmpl = NULL, fn_local = NULL, package, verbose = FALSE) {
  ...
  if (!is.null(fn_tmpl)) {
    ...
    if (file.exists(fn_tmpl)) {
      ...
      # Try to find fuzzy match using existing infrastructure
      ...
      if (length(found_tmpl) > 0) {
        ...
        actual_tmpl <- found_tmpl[1]  # Use first match if multiple
      } else {
        # No match at all
        return(list(
    ...
    # Store the fuzzy match information for later use
  ...
  if (!is.null(fn_local)) {
    ...
    if (file.exists(fn_local)) {
      ...
      if (length(found_local) > 0) {
        ...
        actual_local <- found_local[1]  # Use first match if multiple
      } else {
        # No match at all
        return(list(
...
#' Smart File Pairing Main Function
#'
#' Attempts to intelligently pair template and local configuration files when
#' only one is provided. Uses pattern recognition and existing file discovery
#' infrastructure to find corresponding files. Throws exceptions if pairing fails.
#'
#' @param fn_tmpl Character string with template file path/name (or NULL)
#' @param fn_local Character string with local file path/name (or NULL)  
#' @param package Character string with package name
#' @param verbose Logical, whether to show detailed messages
#' @return List with fn_tmpl, fn_local, and source information
#' @keywords internal
.smart_pair_files <- function(fn_tmpl = NULL, fn_local = NULL, package, verbose = FALSE) {
  
  # Use the check function first
  check_result <- .check_file_pairing(fn_tmpl, fn_local, package, verbose)
  
  # Handle different statuses
  if (check_result$status == "both_provided") {
    if (verbose) {
      .icy_text("Both template and local files provided explicitly")
    }
    return(list(
      fn_tmpl = check_result$fn_tmpl,
      fn_local = check_result$fn_local,
      source = "provided"
    ))
  }
  
  if (check_result$status == "use_defaults") {
    if (verbose) {
      .icy_text("No custom files specified, using defaults")
    }
    return(list(
      fn_tmpl = NULL,
      fn_local = NULL,
  ...
  if (check_result$status == "pair_found") {
    if (verbose) {
      missing_type <- if (is.null(fn_tmpl)) "template" else "local"
      .icy_success(paste0("Auto-detected corresponding ", missing_type, " file: ", 
                         basename(if (missing_type == "template") check_result$fn_tmpl else check_result$fn_local)))
    }
    return(list(
      fn_tmpl = check_result$fn_tmpl,
      fn_local = check_result$fn_local,
  ...
  if (check_result$status == "missing_pair") {
    # Throw exception with detailed suggestions
    suggestions <- .suggest_missing_files(check_result$missing_type, package, 
                                        if (check_result$missing_type == "local") fn_tmpl else fn_local)
    provided_file <- if (check_result$missing_type == "local") fn_tmpl else fn_local
    .icy_stop(paste0(
      "No corresponding ", check_result$missing_type, " file found for ", 
      if (check_result$missing_type == "local") "template" else "local", ": ", provided_file, "\n",
      suggestions
    ))
  }
}
