...
load_config <- function(package = get_package_name(),
  ...
  # Input validation
  if (!is.list(unset)) {
    .icy_stop("unset must be a named list")
  }
  ...
  template_config <- tryCatch({
    ...
    .icy_stop(paste0("Failed to read template configuration: ", e$message))
  })
  ...
  if (length(unset) > 0) {
    ...
    if (is.null(unset_var_names) || any(unset_var_names == "")) {
      .icy_stop("All elements in unset must be named")
    }
  ...
  if (ensure_local && origin %in% c("local", "priority")) {
    ...
    if (is.null(local_file)) {
      ...
      tryCatch({
        ...
        .icy_stop(paste0("Failed to create local config: ", e$message))
      })
    }
  }
  ...
  config <- tryCatch({
    ...
    .icy_stop(paste0("Failed to read ", origin, " configuration: ", e$message))
  })
